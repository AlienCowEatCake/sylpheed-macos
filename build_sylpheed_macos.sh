...
CPPFLAGS_TARGET="${CPPFLAGS}"
export CXXFLAGS="${CFLAGS}"
CXXFLAGS_TARGET="${CFLAGS_TARGET}"
export OBJCFLAGS="${CFLAGS}"
OBJCFLAGS_TARGET="${CFLAGS_TARGET}"

function target_build_env {
...
sed -i '' 's|\(ninja/releases/download/\)v1\.10\.2|\1v1.13.2|' ./gtk-osx-setup.sh
...
EOF
    ...
    clang "${SOURCE_DIR}/enchant/enchant.m" ${CPPFLAGS_TARGET} ${OBJCFLAGS_TARGET} -Wall -Wextra \
        -dynamiclib -fPIC -current_version 11.14.0 -compatibility_version 11.0.0 \
...
setup_sdk(target="${MACOSX_DEPLOYMENT_TARGET}", architectures=["${ARCH}"])
setup_release()
EOF
    ...
    cd qdbm-1.8.78
    target_build_env arch -${ARCH} jhbuild run \
        ./configure --prefix="${HOME}/gtk/inst" --enable-stable --enable-pthread --enable-zlib --enable-iconv
    for i in $(cat Makefile.in | grep -E '^MYLIBOBJS = ' | sed 's|MYLIBOBJS = || ; s|\.o||g') ; do
        target_build_env arch -${ARCH} jhbuild run \
            clang ${CPPFLAGS_TARGET} ${CFLAGS_TARGET} \
                -mmacos-version-min=${MACOSX_DEPLOYMENT_TARGET} -arch ${ARCH} \
                -DQDBM_STATIC -DMYPTHREAD -DMYZLIB -DMYICONV -fPIC -I. \
                -c "${i}.c" -o "${i}.o"
    done
    target_build_env arch -${ARCH} jhbuild run \
        ar rcs "${HOME}/gtk/inst/lib/libqdbm.a" *.o
...
EOF
    ...
    arch -${ARCH} strip "${HOME}/gtk/inst/bin/sylfilter"
    cd ..

    cd sylpheed-3.8.0beta1
    cd macosx/bundle
    sed -i '' 's|\(</main-binary>\)|\1\n  <binary>${prefix}/bin/sylfilter</binary>|' sylpheed.bundle
    arch -${ARCH} jhbuild run gtk-mac-bundler sylpheed.bundle
    ...
    cd ..
    mv "${HOME}/Desktop/Sylpheed.app" "${HOME}/Desktop/Sylpheed_${ARCH}.app"
...
mv "${HOME}/Desktop/Sylpheed_x86_64.app" "${HOME}/Desktop/Sylpheed.app"
...
clang "${SOURCE_DIR}/launcher/launcher.m" \
    -mmacos-version-min=${MACOSX_DEPLOYMENT_TARGET} -arch x86_64 -arch arm64 \
    ${CPPFLAGS_TARGET} ${OBJCFLAGS_TARGET} -Wall -Wextra -framework Foundation \
    -o "${HOME}/Desktop/Sylpheed.app/Contents/MacOS/${BUNDLE_EXECUTABLE}"
